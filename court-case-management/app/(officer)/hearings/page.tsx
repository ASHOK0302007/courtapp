...
export default async function OfficerHearingsPage() {
  const hearings = await listHearings();

  return (
    <div>
      <h1 className="font-display text-2xl font-medium text-ink-900">Hearing calendar</h1>
      <p className="mt-1 text-sm text-ink-500">Every hearing scheduled across your case queue.</p>
              ...
              {hearings.map((h) => (
                <li key={h.id} className="flex items-center justify-between px-5 py-3 text-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link href={`/officer/cases/${h.case_id}`} className="text-brass-500 hover:underline">
                      View case
                    </Link>
                  </div>
                </li>
